...
`v0.7` is complete. It includes Agent execution, Phoenix trace export, a 36-task evaluation suite, regression diagnostics, YAML release gates, CI, and a standalone Dashboard.
...
The evaluation suite contains 36 deterministic customer-support tasks across refund timing and status, order lookup, delivery tracking, return policy, and human escalation. Three refund tasks are marked critical, so release decisions reflect both aggregate quality and business priority.
...
Open the local URL shown by Vite. The scenario selector switches between the blocked regression run and the passing fixed run. Both views load checked-in artifacts generated by the same evaluation CLI used in CI, rather than hand-authored UI examples.
